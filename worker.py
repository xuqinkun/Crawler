...
class CrawlWorker(QObject):
    ...
    def get_progress(self) -> int:
        if self.total_num == 0:
            ...
            return int(100 * self.completed_num / self.total_num)
        ...
        try:
            ...
            for product in product_uncompleted:
                if not self.is_running:
                    ...
                    return
                self.agent.start_craw(url=product.url, session=session)
                # 更新进度
                if self.get_progress() > last_progress:
                    last_progress = self.get_progress()
                    self.progress_updated.emit(self.username, '爬取中', self.get_progress())
                    self.log_updated.emit(self.username, f"[进度] {self.username} 已完成 {self.get_progress()}%")
